...
//}
	while(1){	
		...
		if(GPIO_ReadInputDataBit(GPIOH,GPIO_Pin_15)==0){ //这里不知道？？？？
				int i=0;
				Delay(0xffff);
...
//							printf("%x\r\n",WIFI_RX_Buff[7]);
							selectedNums = 0;
							if(WIFI_RX_Buff[8]==0x01||WIFI_RX_Buff[8]==0x10||WIFI_RX_Buff[8]==0x00){
								samplePoints =4;
							}
							else if(WIFI_RX_Buff[8]==0x0A||WIFI_RX_Buff[8]==0xA0||WIFI_RX_Buff[8]==0x00){
								
								samplePoints =10;
							}
							else if(WIFI_RX_Buff[8]==0x04||WIFI_RX_Buff[8]==0x40||WIFI_RX_Buff[8]==0x00){
								samplePoints =100;
							}
...
void receiveAD(void){
	  ...
	  printf("selectedNums: %d\r\n",selectedNums);
		...
		//int sum = samplePoints *2*8*1024;//这里设置的是总共采样的点数
		int sum = samplePoints*5*selectedNums*1024;//这里设置的是总共采样的点数.一通道
    //int sum = samplePoints *2*9*1024;//这里设置的是总共采样的点数
		...
		//char header[1024]={0x41,0x44,0x43,0x03,0x00,0x02,0xc1,0x01,0xFF,0x00,0x0C,0x80,0x00,0x43,0x44,0x41};//8通道
		char header[1024]={0x41,0x44,0x43,0x03,0x00,0x02,0xc1,0x01,0x07,0x00,0x0C,0x80,0x00,0x43,0x44,0x41};//3通道
