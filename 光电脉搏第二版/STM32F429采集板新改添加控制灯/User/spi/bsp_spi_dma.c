...
/**
  ...
  * @retval 
  */
void spi_dma_config(int nums)
{
		if(TX_Buff!=NULL||RX_Buff!=NULL) {
				free(TX_Buff);
				free(RX_Buff);
		}
		TX_Buff = (uint8_t *) malloc (1024);
	  RX_Buff = (uint8_t *) malloc (1024);
		if(TX_Buff==NULL||RX_Buff==NULL) printf("null\r\n");
...
void spi_dma_config1(int nums)
{
		if(TX_Buff!=NULL||RX_Buff!=NULL) {
				free(TX_Buff);
				free(RX_Buff);
		}
		TX_Buff = (uint8_t *) malloc (nums*1024 *2);
	  RX_Buff = (uint8_t *) malloc (nums*1024 *2);
		if(TX_Buff==NULL||RX_Buff==NULL) printf("null\r\n");
		memset(TX_Buff,0,nums*1024*2);
		memset(RX_Buff,0,nums*1024*2);
		//SPI_Cmd(SPI3,ENABLE);
		SPI3_TX_DMA_Config();	//DMA发送配置
		SPI3_RX_DMA_Config();	//DMA接受配置
	  SPI_I2S_DMACmd(SPI3, SPI_I2S_DMAReq_Tx, ENABLE);	//SPI2 TX DMA请求使能.
